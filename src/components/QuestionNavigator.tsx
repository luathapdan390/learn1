    ...
    <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-4 sm:p-5 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-sm font-bold text-slate-900 dark:text-white">
            Question Navigator ({totalQuestions} Questions)
          </h4>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {isSubmitted
              ? 'Click any number to review explanation'
              : `${answeredCount} of ${totalQuestions} questions completed (0.5 pts each)`}
          </p>
        </div>
        ...
        {questions.map((q, idx) => {
          ...
          return (
            <button
              ...
              className={`relative h-10 rounded-xl border text-xs sm:text-sm font-semibold flex items-center justify-center transition-all cursor-pointer ${btnClass} ${
      ...
      <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
        <button
          ...
          className="flex-1 py-2 px-3 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1 transition cursor-pointer"
        >
      ...
      {!isSubmitted ? (
        ...
        >
          ...
          <span>Submit Test & Calculate 10-Point Score</span>
        </button>
      ) : (
        <button
          onClick={onResetTest}
          className="w-full py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 font-semibold text-xs transition flex items-center justify-center gap-2 cursor-pointer"
        >
